class VideosController < ApplicationController
  ...
  def upload_to_wistia
    ...
    cmd = "ffmpeg -i #{video_file.tempfile.path} -i #{Rails.public_path.join('sample_watermark.png').to_s} -filter_complex 'overlay=10:10' -b:v 289K -crf 18 -f #{File.extname(video_file.original_filename).sub(/\A./, '')} #{watermarked_video.path} -y"
    system( cmd )
