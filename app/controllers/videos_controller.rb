class VideosController < ApplicationController
  ...
  def upload_to_wistia
    ...
    cmd = "ffmpeg -i #{video_file.tempfile.path} -i #{Rails.public_path.join('sample_watermark.png').to_s} -filter_complex 'overlay=10:10' #{video_file.video_bitrate ? '-b:v ' + video_file.video_bitrate + 'K' : ''} -crf 18 -strict -2 -f #{File.extname(video_file.original_filename).sub(/\A./, '')} #{watermarked_video.path} -y"
    system( cmd )
